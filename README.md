...
The naive approach would be to use last known good values. To account for slow drift we can deploy classical statistical tools like Exponential Weighted Moving Average, but this is lagging indicator by nature therefore won't be robust when dealing with sudden shifts (though it is very fast to compute, so maybe it still has it's place in the stack?). However, we can improve on both ideas by using a more sophisticated approach:
...
Instead of deploying the raw, resource-heavy neural network into the SSD's micro-controller, we can distill the neural network’s findings into lightweight mathematical formulas or polynomial regression models. The FTL firmware uses these ML-generated coefficients to proactively adjust the initial search bounds and starting midpoints.
...
- see `glossary.md` for the raw braindump or first commit to see the intuitive kind of thinking (some of it was valuable, some of it was borderline primitive)
